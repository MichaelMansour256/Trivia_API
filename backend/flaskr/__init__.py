...
def create_app(test_config=None):
  ...
  def set_headers(response):
    ...
    response.headers.add('Access-Control-Allow-Methods','GET,POST,DELETE,UPDATE,OPTIONS,PUT')
    return response
  '''
  @TODO(done): 
  ...
  def get_questions():
    ...
    questions_page = Question.query.order_by(Question.id).paginate(page,QUESTIONS_PER_PAGE,error_out=False).items
  ...
  def search_question():
    ...
    try:
        ...
        question_list=Question.query.filter(Question.question.like(f'%{search_term}%')).all()
  ...
  def quiz():
    ...
    try:
      ...
      questions_list = Question.query.filter_by(category=category).filter(Question.id.notin_((previous_questions))).all()
      if (len(questions_list) > 0):
        random_question = questions_list[random.randint(0, len(questions_list))].format()
      else:
